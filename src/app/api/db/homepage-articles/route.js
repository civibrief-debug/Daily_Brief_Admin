...
export async function GET() {
  try {
    ...
    if (db && Array.isArray(db.homepageArticleSections) && db.homepageArticleSections.length > 0) {
      return NextResponse.json({ success: true, data: db.homepageArticleSections });
    }

    return NextResponse.json({ success: true, data: INITIAL_HOMEPAGE_ARTICLE_SECTIONS });
  } catch (err) {
    return NextResponse.json({ success: true, data: INITIAL_HOMEPAGE_ARTICLE_SECTIONS });
  }
}
...
export async function POST(req) {
  try {
    ...
    // 1. Write to shared_database.json (immediate disk sync between admin and reader apps)
